...
from earlyexitnet.tools import \
    MNISTDataColl,CIFAR10DataColl, CIFAR100DataColl, load_model,path_check
...
def get_exits(model_str):
    """
    Get the number of exits for the chosen model.
    """
...
def get_save_path(model_name, notes_path, timestamp=True, desc=None, filetype='json'):
...
def test_only(args):
    ...
    if args.dataset == 'mnist':
        datacoll = MNISTDataColl(
            batch_size_test=batch_size_test,num_workers=num_workers,pin_mem=pin_mem)
    elif args.dataset == 'cifar10':
        datacoll = CIFAR10DataColl(
            batch_size_test=batch_size_test,num_workers=num_workers,
            no_scaling=args.no_scaling,pin_mem=pin_mem)
    elif args.dataset == 'cifar100':
        datacoll = CIFAR100DataColl(
            batch_size_test=batch_size_test,num_workers=num_workers,
            no_scaling=args.no_scaling,pin_mem=pin_mem)
    else:
    ...
    if args.notes_path is not None:
        ...
        notes_path = f'./outputs/{args.model_name}_{args.dataset}/jsons/'
    ...
    if args.threshold_range is not None and args.threshold_step is not None:
        if len(args.threshold_range) == 2:
            ...
            raise NameError(f"Invalid amount of value for the threshold range: \
                            {len(args.threshold_range)}")
    else:
...
def test_multiple(datacoll,model,exits,loss_f,notes_path,args):
    """Run multiples tests on a given model, varying the ealy-exit threshold linearly
    in a given range,with a given step. Save the test results in JSON file
    ...
    for i, thresh in enumerate(np.arange(min_thr, max_thr, step)):
        rt_string = dt.utcfromtimestamp(
            timedelta(seconds=int(running_time)).total_seconds()
        ).strftime("%M:%S")
        tt_string = dt.utcfromtimestamp(
            timedelta(seconds=total_time).total_seconds()
        ).strftime("%M:%S")
        print(f"\nRunning test {i}/{num_tests}, thresh: {thresh} \
              \t\t [{rt_string}/{tt_string}]")

        # ignore entropy threshold
        elapsed_time, test_stats = run_test(
            datacoll, model, exits, [float(thresh)], None, loss_f, args)
...
def run_test(datacoll, model, exits, top1_thr, entr_thr, loss_f, args, save_raw=False):
    ...
    Args:
        ...
        loss_f : Loss function (unused)
        args : CLI arguments
        save_raw (bool, optional): If True, final activation layer and softmax vectors are
            saved in return dict. Defaults to False.

    Returns:
        _tuple[float, dict]_: Elapsed time and dictionary containing all test information
    """
...
def test_single(datacoll, model, exits, loss_f, notes_path, args):
    ...
    elapsed_time, test_stats = run_test(
        datacoll, model, exits,
        args.top1_threshold, args.entr_threshold, loss_f,
        args, save_raw)

    print(f"top1 thrs: {args.top1_threshold},  entropy thrs: {args.entr_threshold}")
    print(f"Total time elapsed: {elapsed_time}s")

    ts = dt.now().strftime("%Y-%m-%d_%H%M%S")
    if not save_raw: # when saving raw output, txt file doesn't make sense
        with open(
            get_save_path("test_notes", notes_path, filetype='txt', timestamp=False),
            'a') as notes:
    ...
    save_path = get_save_path(
        args.model_name, os.path.split(notes_path)[0], desc="single")
...
def train_n_test(args):
    ...
    path_str = f'outputs/{args.model_name}_{args.dataset}/'
    pretrain_backbone=True
    if args.bb_epochs == 0:
        # if no model provided, joint from scratch
        pretrain_backbone=False

    print(f"backbone epochs: {args.bb_epochs} joint epochs: {args.jt_epochs}")
...
def main():
    ...
    # this formatter prevents the help text description automatically
    # wrapping to fit in terminal requires the help text to have
    # manual line breaks
    ...
    parser.add_argument('-bbe','--bb_epochs', metavar='N',
                        type=int, default=0, required=False,
            help='Epochs to train backbone separately, or non ee network')
    parser.add_argument('-jte','--jt_epochs', metavar='n',
                        type=int, default=0, required=False,
            help='epochs to train exits jointly with backbone')
    parser.add_argument('-exe','--ex_epochs', metavar='n',
                        type=int, default=0, required=False,
            help='epochs to train exits with frozen backbone')
    parser.add_argument('-vf','--validation_frequency',
                        type=int,default=1,required=False,
            help='Validation and save frequency. Number of epochs to wait for \
                        before valid,saving.')
    # opt selection
    parser.add_argument('-bbo', '--bb_opt_cfg', type=str,
                        default='adam-brn', required=False,
            help='Selection string to pick backbone optimiser configuration \
                        from training_tools')
    parser.add_argument('-jto', '--jt_opt_cfg', type=str,
                        default='adam-brn', required=False,
            help='Selection string to pick joint optimiser configuration \
                        from training_tools')
    parser.add_argument('-exo', '--ex_opt_cfg', type=str,
                        default='adam-brn', required=False,
            help='Selection string to pick exit-only optimiser configuration \
                        from training_tools')
    # run notes
    parser.add_argument('-rn', '--run_notes', type=str, required=False,
            help='Some notes to add to the train/test information about \
    ...
    parser.add_argument('-cf','--confidence_function', required=False, nargs='+', type=int,
            help='Choose which function to be used when determining the confidence \
                 of the network at a given exit, pick one or many. \
                 \n0 Entropy\n1 Softmax\n2 Trunc Base-2 Softmax\n3 \
                 Non-Trunc Base-2 Softmax\n4 Base-2 Sub-Softmax\n')


    #parser.add_argument(
    #'--seed', metavar='N', type=int, default=random.randint(0,2**32-1),
    ...
    parser.add_argument(
        '-sr', '--save_raw', action=argparse.BooleanOptionalAction,
    ...
    parser.add_argument('-go', '--generate_onnx',metavar='PATH',type=path_check,
                        required=False,
                        help='Generate onnx from loaded or trained Pytorch model, \
                        specify the directory of the output onnx')
