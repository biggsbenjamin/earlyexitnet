...
from earlyexitnet.tools import MNISTDataColl, CIFAR10DataColl, load_model, path_check, save_model
from earlyexitnet.onnx_tools.onnx_helpers import to_onnx


def get_exits(model_str):
    '''
    Get the number of exits for a given model.
    '''
    # NOTE only used if there is no exit num constant
    # set number of exits
    if model_str in ["lenet", "testnet", "brnfirst", "brnsecond", "brnfirst_se", "backbone_se"]:
        exits = 1
    elif model_str in ["b_lenet", "b_lenet_fcn", "b_lenet_se", "b_lenet_cifar",
                       "b_lenet_imagenet"]:
        exits = 2
    else:
...
def test_only(args):
    '''
    CLI function for running test on pretrained model.
    '''
    ...
    if args.top1_threshold is None and args.entr_threshold is None and exits > 1:
...
def test(datacoll, model, exits, loss_f, save_path, notes_path, args):
    # check if there are thresholds provided
    if args.top1_threshold is None and args.entr_threshold is None and exits > 1:
        # no thresholds provided, skip testing
        print("WARNING: No Thresholds provided, skipping testing.")
        return
    if args.top1_threshold is None:
    ...
    if exits > 1:
        if len(args.top1_threshold) + 1 != exits or len(args.entr_threshold) + 1 != exits:
        ...
        net_test = Tester(model, test_dl, loss_f, exits, top1_thr, entr_thr)
    else:
    ...
    print(f"top1 thrs: {top1_thr},  entropy thrs: {entr_thr}")
...
def train_n_test(args):
    '''
    Function for that sets up the training for a given model
    and dataset. Then runs the model with the test dataset.
    '''
    ...
    if exits > 1:
        ...
        best, last = net_trainer.train_backbone(internal_folder=intr_path)
...
def build_model_only(args):
    ...
    model_path = save_model(model, f"outputs/build-model-only/{args.model_name}",
                            "build-model-only")
...
def main():
    ...
    parser.add_argument("-m", "--model_name", required=True,
                        help="select the model name - see training model")
    ...
    parser.add_argument(
        "-bstr", "--batch_size_train", type=int, default=500,
    ...
    parser.add_argument(
        "-bste", "--batch_size_test", type=int, default=1,
    ...
    if args.generate_onnx is not None:
        ...
        to_onnx(model, shape, batch_size=1, path=args.generate_onnx, fname=fname)
